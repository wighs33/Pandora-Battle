...
bool USkillActorFieldAction::ShouldSkipStaticDurationMovementLock() const
{
    return Settings.StaticActorClass
       && Settings.StaticActorClass.Get()->IsChildOf(AAnimeAuraActor::StaticClass());
}
...
AActor* USkillActorFieldAction::SpawnStaticActorForSocket(const FName SocketName)
{
    ...
    const bool bShouldReplicateSpawnedActor =
       Settings.bForceReplicateSpawnedActor || SpawnedActor->IsA<AEffectAreaBase>();
    ...
    {
       AnimeAuraActor->ConfigurePresentationSettings(Settings.AnimeAuraPresentation);
    }
...
void USkillActorFieldAction::BindStaticTriggerDamage(AActor* SpawnedActor)
{
    ...
    if (SpawnedActor->IsA<AOmenOrbGlitchActor>() && Settings.bOmenOrbApplyFinishAreaDamage)
    {
       return;
    }
