...
AGroundTargetActor::AGroundTargetActor(const FObjectInitializer& ObjectInitializer)
...
void AGroundTargetActor::Tick(const float DeltaSeconds)
{
...
void AGroundTargetActor::BeginPlay()
{
	Super::BeginPlay();
...
void AGroundTargetActor::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
FHitResult AGroundTargetActor::PerformTrace(AActor* InSourceActor)
{
...
void AGroundTargetActor::DestroySpawnedDecal()
{
...
void AGroundTargetActor::ApplyDecalSize(const double InDecalSize) const
{
	if (!IsValid(SpawnedDecalComponent))
	{
		return;
...
void AGroundTargetActor::ApplyGroundHitToDecal(const FHitResult& GroundHit)
{
...
void AGroundTargetActor::ApplyCachedGroundHitToDecal() const
{
...
void AGroundTargetActor::SetTargetingDecalVisible(const bool bVisible) const
{
	if (IsValid(SpawnedDecalComponent))
	{
...
void AGroundTargetActor::UpdateDecalGrowth()
{
...
void AGroundTargetActor::MarkGroundTraceFailure()
{
	bLastGroundTraceSucceeded = false;
	bHasLastGroundDecalLocation = false;
}

void AGroundTargetActor::MarkGroundTraceSuccess()
{
